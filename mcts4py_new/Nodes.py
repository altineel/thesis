...
from mcts4py_new.Types import TAction, TState, TRandom
...
class NewNode(ABC, Generic[TRandom, TAction]):
    ...
    def name(self):
        if self.inducing_action is None:
            return f'None_{self._state.port}'
        #return f'F{int(self.state.fuel_amount)}_{self.inducing_action.name}_{self.state.port}_{round(self.state.price)}'
        return f'{int(self.state.fuel_amount)}{self.inducing_action.name}{self.state.port}{round(self.state.price)}'
