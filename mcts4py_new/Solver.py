...
from mcts4py_new.Types import *
from mcts4py_new.Nodes import *
...
class MCTSSolver(ABC, Generic[TAction, TNode, TRandom]):
    ...
    def visualize_tree(self):
        ...
        def add_nodes(node):
            parent_name = 'D' + node.name if isinstance(node, DecisionNode) else 'R' + node.name
            added_edges[parent_name] = []
            for child in node.children:
                ...
                if name not in added_edges[parent_name]:
                    dot.edge(parent_name, name)
                    added_edges[parent_name].append(name)
                add_nodes(child)

        root = self.root()
        root_name = 'D' + root.name
        dot.node(root_name, label=f"{root_name}\n({root.n})")
        add_nodes(root)
